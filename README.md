...
Version 1.1.0 embeds the official `sunniesnow-record` v0.5.4 release at commit `bcb045c`, including the updated Sunniesnow game engine and the big-text quality option, and adds cover generation. The build and package verification both check the embedded recorder version, and Git-based builds also verify its exact release commit.

The main workflow exposes chart and difficulty selection, quality, nickname and avatar, output filename, format, and location. Available formats are MKV, MP4, WebM, MOV, AVI, and MPEG-TS. Cover generation is a dedicated main-workflow section: cover options have their own localized UI (Simplified Chinese, English, and Japanese follow the system language), and the "Generate Cover" action invokes the embedded `sunniesnow-cover-gen` to render a PNG cover from the current chart, difficulty, background, skin, and profile settings (same name as the video output, with a `.png` extension). Every remaining `sunniesnow-record` option is available once in the advanced settings, including independent sizes for Tap, Drag, Flick, Hold, Drag-Flick, head-only Hold, and background notes, plus big-text quality.
