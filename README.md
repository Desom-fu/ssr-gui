...
Version 0.3.3 pins the final `sunniesnow-record` v0.5.1 release. The build and package verification both check the embedded recorder version, and Git-based builds also verify its exact release commit. This release preserves the packaged recorder's WebGL fallback, keeps node-side asset URLs alive long enough for slower Windows machines, and now also completes recording in canvas mode when headless WebGL is unavailable.
