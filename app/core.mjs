...
export const RECORDER_FIELDS = Object.freeze([
	...
	field("speed", "number", 2, { min: 0, max: 20, step: 0.1 }), field("tipPointSpeed", "number", 1, { min: 0, max: 20, step: 0.1 }), field("tipPointDistance", "number", 1, { min: 0, max: 20, step: 0.1 }),
	field("noteSizeTap", "number", 0.95, { min: 0, step: 0.05 }), field("noteSizeDrag", "number", 0.65, { min: 0, step: 0.05 }), field("noteSizeFlick", "number", 0.95, { min: 0, step: 0.05 }), field("noteSizeHold", "number", 0.95, { min: 0, step: 0.05 }), field("noteSizeDragFlick", "number", 0.65, { min: 0, step: 0.05 }), field("noteSizeHeadOnlyHold", "number", 0.95, { min: 0, step: 0.05 }), field("noteSizeBgNote", "number", 0.95, { min: 0, step: 0.05 }),
	...
	field("scrollJudgementLine", "number", 0.8, { min: 0, max: 1, step: 0.05 }), field("scrollDistance", "number", 1, { min: 0, max: 10, step: 0.1 }), field("hideTipPoints", "boolean", false), field("hideNotes", "boolean", false), field("hideCircles", "boolean", false), field("hideBgNotes", "boolean", false), field("hideFx", "boolean", false), field("hideBgPattern", "boolean", false), field("fadingStart", "number", 1, { min: 0, max: 10, step: 0.05 }), field("fadingDuration", "number", 0.2, { min: 0, max: 10, step: 0.05 }), field("qualityBigText", "number", 1, { min: 0.05, max: 8, step: 0.05 }), field("reverseNoteOrder", "boolean", false), field("circleMovesWithNote", "boolean", false), field("disableOrnament", "boolean", false),
...
// Keep this list in lockstep with sunniesnow-record/cover-gen.mjs DEFAULT_SETTINGS.
export const COVER_FIELDS = Object.freeze([
	field("levelFile", "select", "upload", { options: ["upload", "online"] }), field("levelFileOnline", "text", ""), field("levelFileUpload", "file", null, { accept: ".ssc,application/zip" }), field("chartSelect", "text", null),
	field("background", "select", "from-level", { options: ["from-level", "online", "upload"] }), field("backgroundOnline", "text", "default.svg"), field("backgroundFromLevel", "text", null), field("backgroundUpload", "file", null, { accept: "image/*" }), field("backgroundBlur", "number", 20, { min: 0, max: 100, step: 1 }), field("backgroundBrightness", "number", 0.5, { min: 0, max: 1, step: 0.05 }),
	field("skin", "select", "default", { options: ["default", "online", "upload"] }), field("skinOnline", "text", ""), field("skinUpload", "file", null, { accept: ".ssp,application/zip" }),
	field("nickname", "text", "New Poet"), field("avatar", "select", "online", { options: ["online", "upload", "gravatar"] }), field("avatarOnline", "text", "default.svg"), field("avatarUpload", "file", null, { accept: "image/*" }), field("avatarGravatar", "text", ""),
	field("width", "number", 1920, { min: 320, max: 7680, step: 1 }), field("height", "number", 1080, { min: 180, max: 4320, step: 1 }), field("avoidDownloadingFonts", "boolean", false), field("plugin", "array", []), field("pluginOnline", "array", []), field("pluginUpload", "file-array", [], { accept: ".ssp,application/zip" }),
	field("help", "boolean", false), field("quiet", "boolean", false), field("suppressWarnings", "boolean", false), field("tempDir", "path", null), field("coverThemeImageX", "number", null, { nullable: true }), field("coverThemeImageY", "number", null, { nullable: true }), field("coverThemeImageWidth", "number", null, { nullable: true }), field("output", "path", "output.png"), field("clean", "boolean", false),
]);

export const COVER_DEFAULTS = Object.freeze(Object.fromEntries(COVER_FIELDS.map(item => [item.key, item.defaultValue])));
// Cover settings already covered by recorder fields reuse the recorder controls;
// only cover-specific options need dedicated controls, which live in the
// dedicated main-workflow cover section instead of the advanced settings.
const coverFieldKeySet = new Set(RECORDER_FIELDS.map(item => item.key));
export const COVER_EXTRA_FIELDS = Object.freeze(COVER_FIELDS.filter(fieldDefinition => !coverFieldKeySet.has(fieldDefinition.key)));

// Localized display strings for the dedicated cover section. Keys are stable
// identifiers; each locale maps them to UI text.
export const COVER_I18N_LOCALES = Object.freeze(["zh-CN", "en-US", "ja-JP"]);
export const COVER_I18N = Object.freeze({
	"zh-CN": Object.freeze({
		coverTitle: "封面",
		coverGenerate: "生成封面",
		coverThemeImageX: "主题图中心 X",
		coverThemeImageY: "主题图中心 Y",
		coverThemeImageWidth: "主题图宽度",
		coverThemeImageHint: "留空则使用游戏默认布局",
	}),
	"en-US": Object.freeze({
		coverTitle: "Cover",
		coverGenerate: "Generate Cover",
		coverThemeImageX: "Theme Image Center X",
		coverThemeImageY: "Theme Image Center Y",
		coverThemeImageWidth: "Theme Image Width",
		coverThemeImageHint: "Leave empty to use the default game layout",
	}),
	"ja-JP": Object.freeze({
		coverTitle: "ジャケット",
		coverGenerate: "ジャケットを生成",
		coverThemeImageX: "テーマ画像中心 X",
		coverThemeImageY: "テーマ画像中心 Y",
		coverThemeImageWidth: "テーマ画像の幅",
		coverThemeImageHint: "空欄の場合はゲームのデフォルト配置を使用",
	}),
});
export const FIELD_LABELS = Object.assign(Object.fromEntries(RECORDER_FIELDS.map(item => [item.key, item.key])), {
	levelFile: "谱面来源", levelFileOnline: "在线谱面名", levelFileUpload: "谱面文件", musicSelect: "音乐文件", chartSelect: "难度文件", lyrica5: "Lyrica 5 判定", speed: "音符速度", tipPointSpeed: "引导点速度", tipPointDistance: "引导点距离", noteSizeTap: "Tap 大小", noteSizeDrag: "Drag 大小", noteSizeFlick: "Flick 大小", noteSizeHold: "Hold 大小", noteSizeDragFlick: "Drag-Flick 大小", noteSizeHeadOnlyHold: "仅头 Hold 大小", noteSizeBgNote: "背景音符大小", background: "背景来源", backgroundOnline: "在线背景", backgroundFromLevel: "谱面背景", backgroundUpload: "背景文件", backgroundBlur: "背景模糊", backgroundBrightness: "背景亮度", skin: "皮肤来源", skinOnline: "在线皮肤", skinUpload: "皮肤插件", fx: "特效来源", fxOnline: "在线特效", fxUpload: "特效插件", hudTopCenter: "顶部中间 HUD", hudTopLeft: "顶部左侧 HUD", hudTopRight: "顶部右侧 HUD", doubleLineTap: "点按双线", doubleLineHold: "长押双线", doubleLineDrag: "拖键双线", doubleLineFlick: "划键双线", doubleLineDragFlick: "拖划双线", forceDoubleLine: "强制双线", opacityFake: "Fake 透明度", hideFxInFront: "隐藏前景特效", hideFxPerfect: "隐藏 Perfect 特效", hideFxHoldStart: "隐藏长押起始特效", scrollJudgementLine: "滚动判定线", scrollDistance: "滚动距离", hideTipPoints: "隐藏引导点", hideNotes: "隐藏音符", hideCircles: "隐藏圆环", hideBgNotes: "隐藏背景音符", hideFx: "隐藏特效", hideBgPattern: "隐藏背景图案", fadingStart: "淡出开始", fadingDuration: "淡出时长", qualityBigText: "大字画质", reverseNoteOrder: "反转音符顺序", circleMovesWithNote: "圆环跟随音符", disableOrnament: "禁用装饰", se: "音效来源", seOnline: "在线音效", seUpload: "音效插件", volumeSe: "音效音量", volumeMusic: "音乐音量", delay: "音频延迟", scroll: "滚动模式", chartOffset: "谱面偏移", gameSpeed: "游戏速度", horizontalFlip: "水平翻转", verticalFlip: "垂直翻转", start: "开始比例", end: "结束比例", beginningPreparationTime: "开头准备时间", nickname: "昵称", avatar: "头像来源", avatarOnline: "在线头像", avatarUpload: "头像文件", avatarGravatar: "Gravatar 邮箱", width: "视频宽度", height: "视频高度", avoidDownloadingFonts: "使用系统字体", plugin: "插件来源", pluginOnline: "在线插件", pluginUpload: "插件文件", help: "打印帮助后退出", fps: "帧率", quiet: "静默输出", suppressWarnings: "抑制警告", tempDir: "临时目录", output: "输出文件", resultsDuration: "结算画面时长", waitForMusic: "等待音乐结束", clean: "清理缓存", ffmpeg: "FFmpeg 路径", ffmpegOptions: "FFmpeg 输入参数", ffmpegOutputOptions: "FFmpeg 输出参数",
});

// Fields represented by dedicated controls in the main workflow must not be
// rendered a second time under "all recorder parameters".
export const MAIN_FIELD_KEYS = Object.freeze([
	"levelFileUpload", "chartSelect", "speed", "nickname", "avatar", "avatarOnline", "avatarUpload", "avatarGravatar",
	"width", "height", "fps", "avoidDownloadingFonts", "output", "resultsDuration", "waitForMusic",
]);
const mainFieldKeySet = new Set(MAIN_FIELD_KEYS);
export const ADVANCED_RECORDER_FIELDS = Object.freeze(RECORDER_FIELDS.filter(fieldDefinition => !mainFieldKeySet.has(fieldDefinition.key)));
...
export function replaceOutputFilename(outputPath, filename, pathApi, format = "mkv") {
	const requested = String(filename || "").trim();
	if (!requested) return "";
	const normalized = replaceOutputExtension(pathApi.basename(requested), pathApi, format);
	const current = String(outputPath || "").trim();
	return current ? pathApi.join(pathApi.dirname(current), normalized) : normalized;
}
...
export function buildCoverArgs(settings) {
	if (!String(settings.cliPath || "").trim()) throw new TypeError("cliPath is required.");
	if (!String(settings.output || "").trim()) throw new TypeError("output is required.");
	const values = { ...COVER_DEFAULTS, ...settings };
	values.levelFileUpload ||= settings.levelPath;
	values.tempDir ||= settings.tempDir;
	if (values.levelFile === "upload" && !String(values.levelFileUpload || "").trim()) throw new TypeError("levelFileUpload is required for upload mode.");
	if (values.levelFile === "online" && !String(values.levelFileOnline || "").trim()) throw new TypeError("levelFileOnline is required for online mode.");
	for (const definition of COVER_FIELDS) {
		if (definition.type !== "number") continue;
		if (definition.nullable && (values[definition.key] == null || values[definition.key] === "")) {
			values[definition.key] = null;
			continue;
		}
		values[definition.key] = finiteNumber(values[definition.key], definition.key, definition);
	}
	const args = [settings.cliPath];
	for (const definition of COVER_FIELDS) {
		const converted = valueFor(definition, values[definition.key]);
		for (const item of (Array.isArray(converted) ? converted : [converted])) {
			...
			args.push(`--${slug(definition.key)}`, item);
		}
	}
	return args;
}
export function progressFromCoverOutput(line, previous = 0) {
	const text = String(line || "").toLowerCase();
	if (text.includes("done!")) return 100;
	if (text.includes("finalizing")) return Math.max(previous, 60);
	if (text.includes("loading modules")) return Math.max(previous, 30);
	if (text.includes("loading")) return Math.max(previous, 10);
	return previous;
}
