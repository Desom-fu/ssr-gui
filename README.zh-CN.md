...
1.1.0 版内置官方 `sunniesnow-record` v0.5.4（commit `bcb045c`），包含更新后的 Sunniesnow 游戏引擎以及大字画质选项，并新增了封面生成功能。构建与成品包校验都会检查内置 recorder 的版本；使用 Git 源码构建时还会核对其精确 release commit。

主界面直接提供谱面、难度、画质、昵称、头像、输出文件名、格式与位置。可选格式包括 MKV、MP4、WebM、MOV、AVI 和 MPEG-TS。「封面」是主界面的独立板块：封面参数拥有独立的本地化界面（跟随系统语言提供简体中文、English、日本語），点击「生成封面」会调用内置 recorder 的 `sunniesnow-cover-gen`，按当前谱面、难度、背景、皮肤与用户设置直接生成 PNG 封面（与视频输出同名，扩展名为 `.png`）。其余每个 `sunniesnow-record` 参数只在更多设置中出现一次，包括 Tap、Drag、Flick、Hold、Drag-Flick、仅头 Hold 与背景音符的独立大小以及大字画质。
