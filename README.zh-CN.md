...
0.3.0 版精确固定到 `sunniesnow-record` 最终发布的 v0.5.1。构建与成品包校验都会检查内置 recorder 的版本；使用 Git 源码构建时还会核对最终 release commit。
...
配置工具栏支持保存和导入 JSON 配置。程序会在启动时从应用数据目录恢复上次配置，默认自动保存；也可以在保存/导入按钮旁关闭自动保存。
