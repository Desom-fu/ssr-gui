...
0.3.1 版精确固定到 `sunniesnow-record` 最终发布的 v0.5.1。构建与成品包校验都会检查内置 recorder 的版本；使用 Git 源码构建时还会核对最终 release commit。本次发布还加强了打包 recorder 对 `@pixi/node` WebGL 初始化失败的处理，在缺少 headless GL 的 Windows 环境里可以正常回退而不是直接崩溃。
